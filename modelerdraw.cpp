...
void drawTorus(double or, double ir)
{
	...
	{
		_dump_current_modelview();
		fprintf(mds->m_rayFile, "scale(%f,%f,torus {\n", ir, or);
		_dump_current_material();
		fprintf(mds->m_rayFile, "})\n");
	}
	else
		...
		for (int i = 0; i < divisions; i++) {
			...
			for (int j = 0; j <= divisions; j++) {
				for (int k = 1; k >= 0; k--) {
					...
					x = (or + ir * cos(s * 2 * M_PI / divisions)) * cos(t * 2 * M_PI / divisions);
					y = (or + ir * cos(s * 2 * M_PI / divisions)) * sin(t * 2 * M_PI / divisions);
					z = ir * sin(s * 2 * M_PI / divisions);
					glVertex3d(x, y, z);
					glNormal3d(x, y, z); // Normal = Vertice!?
				}
			}
			glEnd();
		}
